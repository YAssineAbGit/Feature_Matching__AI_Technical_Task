...
ORB_matching_result = cv2.drawMatches(img1, orb_kp1, img2, orb_kp2, orb_matches[:100], None, flags = 2)        
# cv.drawMatchesKnn expects list of lists as matches.
SIFT_matching_result = cv2.drawMatchesKnn(img1,sift_kp1,img2,sift_kp2,sift_good[:100],None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
SURF_matching_result = cv2.drawMatchesKnn(img1,surf_kp1,img2,surf_kp2,surf_good[:100],None,flags=cv2.DrawMatchesFlags_NOT_DRAW_SINGLE_POINTS)
...
print('orb    sift    surf')
for i in range(100):
    print(str(orb_matches[i].distance) + '    ' + str(sift_matches[i][0].distance) + '    ' + str(surf_matches[i][1].distance))
